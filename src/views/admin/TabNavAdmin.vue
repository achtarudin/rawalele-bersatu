...
const routeBottomNavs = computed(() => router.options.routes
    .filter((route) => route.meta?.TabNavAdmin == true && route.children && route.children?.length > 0))
                ...
                <ion-tabs @ionTabsWillChange="beforeTabChange" @ionTabsDidChange="afterTabChange"
                    ...
                    <ion-tab-bar slot="bottom">
                        <ion-tab-button v-for="(routeBottomNav, index) in routeBottomNavs[0].children ?? []"
                            ...
                            :class="[routeBottomNav.meta?.pageName == router.currentRoute.value.meta.pageName ? 'tw-text-green-500' : null]"
